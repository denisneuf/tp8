...
class ProductController extends AdminMenuController
{
    ...
    public function save(Request $request)
    {
        ...
        if ($file) {
            try {
                //$savename = Filesystem::putFile('uploads/products', $file);
                ...
                $image->thumb(800, 800)->save($path); // redimensionar

                dump($image);

                // $data['pic'] = '/storage/' . $savename;

                $data['pic'] = $path;

            } catch (\Exception $e) {
                //dump("error");
                return json(['status' => 'error', 'message' => 'Error al subir la imagen']);
            }
        }
        ...
        try {
            ...
            //return json(['status' => 'success', 'message' => 'Producto creado correctamente']);
        } catch (\Exception $e) {
            Session::flash('error', 'Error al guardar el tipo de producto: ' . $e->getMessage());
            //return json(['status' => 'error', 'message' => 'Error al guardar el producto']);
        }
...
public function update(Request $request, $id)
{
    ...
    try {
        ...
        return redirect('/admin/product/edit?id=' . $id)->with('error', $e->getMessage());
    }
}
*/
...
/*
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $special = $request->post('special', []);
            foreach ($special as $fieldId => $value) {
                ProductSpecialValue::updateOrCreate(
                    ['product_id' => $id, 'special_field_id' => $fieldId],
                    ['value' => $value]
                );
            }

            return redirect('product_index')->with('success', 'Producto actualizado correctamente');
        } catch (\Exception $e) {
            //return $this->error('Error al actualizar el producto');
...
*/
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            Product::update($data, ['id' => $id]);
            return json(['status' => 'success', 'message' => 'Producto actualizado correctamente']);
        } catch (\Exception $e) {
            return json(['status' => 'error', 'message' => 'Error al actualizar el producto']);
        }
    }
    */
